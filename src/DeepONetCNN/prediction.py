...
if __name__ == "__main__":
    ...
    model.load_state_dict(torch.load('/app/src/DeepONetCNN/best_model.pt', map_location='cpu', weights_only=True))
    ...
    with torch.no_grad():
        for batch in tqdm(val_loader):
            ...
            real_y = real_y.view(B, R-1, H, W)
            real_pred = real_pred.view(B, R-1, H, W)
            for i in range(B):
                input_file_name = f'input_step_{step}.gif'
                output_file_name = f'output_step_{step}.gif'
                create_gif_from_array(real_y[i].detach().numpy(), os.path.join(out_path, 'result_gifs'), file_name=input_file_name)
                create_gif_from_array(real_pred[i].detach().numpy(), os.path.join(out_path, 'result_gifs'), file_name=output_file_name)
                step += 1
