...
from deeponet_window_dataloader import DeepONetDataset, get_cr_dirs
...
from trainer_window import train, save_training_results_artifacts
...
def main():
    ...
    with open('/app/src/DeepONetBand/config.toml', 'r') as f:
        config = toml.load(f)
    ...
    if enable_wandb_logging:
        ...
        cr_train, cr_test = cr_dirs[:split_ix].tolist(), cr_dirs[split_ix:].tolist()  
    else:
        cr_train, cr_test = cr_dirs[:32].tolist(), cr_dirs[32:64].tolist()

    train_dataset = DeepONetDataset(DATA_DIR, cr_train, scale_up=scale_up, pos_embedding=pos_embedding, trunk_sample_size=trunk_sample_size, transform=data_transform)   
    ...
    with open('/app/src/DeepONetBand/debug.txt', 'w') as f:
        f.write(f'Branch input dims: {train_dataset.get_branch_input_dims()}')
        f.write(f'Trunk input dims: {train_dataset.get_trunk_input_dims()}')
        f.write(f'SIMS shape: {train_dataset.sims[0].shape}')
    ...
    if enable_wandb_logging and accelerator.is_main_process:
        ...
        save_summary(os.path.join('/app/src/DeepONetBand/', "branch_model_summary.txt"), model.branch, input_shape=(2, train_dataset.get_branch_input_dims()),)
        save_summary(os.path.join('/app/src/DeepONetBand/', "trunk_model_summary.txt"), model.trunk, input_shape=(2, train_dataset.get_trunk_input_dims()),)
