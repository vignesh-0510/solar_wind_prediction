...
def update_running_metric(metrics_list, running_dict, loss, real_y, real_pred):
    for k in metrics_list:
        if k == 'loss':
            running_dict[k] += loss
        elif k == 'MSE_MASKED':
            running_dict[k] += AVAILABLE_METRICS_DICT[k](real_y, real_pred, sobel_edge_map(real_y))
        else:
            running_dict[k] += AVAILABLE_METRICS_DICT[k](real_y, real_pred)
    return running_dict
...
def get_training_results(train_metrics_dict, val_metrics_dict):
    training_results = {k: np.array(v) for k, v in train_metrics_dict.items()}
    training_results.update({k: np.array(v) for k, v in val_metrics_dict.items()})
...
def train(
    ...
    val_metrics_dict = {f'val_{metric}': [] for metric in metrics_list}
    ...
    for epoch in range(n_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch+1}/{n_epochs} [Train]", leave=False):
            ...
            # bookkeeping
            cur_loss = loss.item() * y_true.size(0)
            ...
            running_metrics = update_running_metric(metrics_list, running_metrics, cur_loss, real_y, real_pred)
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"Epoch {epoch+1}/{n_epochs} [Val]", leave=False):
                ...
                running_metrics = update_running_metric(metrics_list, running_metrics, cur_loss, real_y, real_pred)
        val_epoch_metrics = get_epoch_metric(metrics_list, running_metrics, len(val_loader.dataset), prefix='val')
        wandb_dict.update(val_epoch_metrics)

        update_metrics_list_dict(metrics_list, val_metrics_dict, val_epoch_metrics)

        if verbose:
            print(
                f"Epoch {epoch+1}:",
                f"Train Loss = {train_epoch_metrics['train_loss']:.6f} | Val Loss = {val_epoch_metrics['val_loss']:.6f}",
                f"Train MSE = {train_epoch_metrics['train_MSE']:.6f} | Val MSE = {val_epoch_metrics['val_MSE']:.6f}",
                f"Train MSE MASKED = {train_epoch_metrics['train_MSE_MASKED']:.6f} | Val MSE MASKED = {val_epoch_metrics['val_MSE_MASKED']:.6f}",
                f"Train PSNR = {train_epoch_metrics['train_PSNR']:.6f} | Val PSNR = {val_epoch_metrics['val_PSNR']:.6f}",
                "================================================================================================",
                sep = '\n'
            )
        if run is not None:
    ...
    training_results = get_training_results(train_metrics_dict, val_metrics_dict)
