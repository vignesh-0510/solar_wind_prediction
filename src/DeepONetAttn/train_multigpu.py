...
from src.DeepONetAttn.deeponet_dataloader import DeepONetDataset, get_cr_dirs
...
def main():
    ...
    with open('/app/src/DeepONetAttn/config.toml', 'r') as f:
        config = toml.load(f)
    ...
    print(f'Starting Attn DeepONet training with {len(cr_train)} train files and {len(cr_test)} test files.')
